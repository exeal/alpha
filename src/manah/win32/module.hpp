...
public:
	...
	bool						writeIntegerProfile(const ::WCHAR* section, const TCHAR* key, ::UINT value);
...
inline ::HRSRC Module::findResource(const ResourceID& id, const ::WCHAR* type) {return ::FindResource(getHandle(), id.name, type);}

inline ::HRSRC Module::findResource(const ResourceID& id, const ::WCHAR* type, ::WORD language) {return ::FindResourceEx(getHandle(), id.name, type, language);}
...
ProfilableApplication<TopWindow>::readStringProfile(const ::WCHAR* section, const ::WCHAR* key, const ::WCHAR* defaultValue /* = L"" */) {
	::WCHAR buffer[1024];
	::GetPrivateProfileStringW(section, key, defaultValue, buffer, countof(buffer), iniFileName_);
	return buffer;
}
