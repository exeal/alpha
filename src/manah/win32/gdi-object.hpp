...
inline bool Bitmap::getBitmap(BITMAP& bitmap) const {return ::GetObjectW(getHandle(), sizeof(HBITMAP), &bitmap) != 0;}
...
inline bool Brush::getLogBrush(LOGBRUSH& logbrush) const {return ::GetObjectW(getHandle(), sizeof(HBRUSH), &logbrush) != 0;}
...
inline bool Font::getLogFont(LOGFONTW& logfont) const {return ::GetObjectW(getHandle(), sizeof(LOGFONTW), &logfont) != 0;}
...
inline bool Pen::getExtLogPen(EXTLOGPEN& extlogpen) const {return toBoolean(::GetObjectW(getHandle(), sizeof(EXTLOGPEN), &extlogpen));}

inline bool Pen::getLogPen(LOGPEN& logpen) const {return toBoolean(::GetObjectW(getHandle(), sizeof(LOGPEN), &logpen));}
