...
void AboutDialog::onInitDialog(HWND, bool&) {
	homePageLink_.create(getHandle(), ::GetModuleHandle(0), IDC_LINK_HOMEPAGE);
	homePageLink_.setText(HOME_PAGE_URL);
	homePageLink_.setPosition(0, 88, 98, 0, 0, SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOZORDER);
	sourceForgeLink_.create(getHandle(), ::GetModuleHandle(0), IDC_LINK_SOURCEFORGE);
	...
	addToolTip(homePageLink_.getHandle(), L"Home page");
	addToolTip(sourceForgeLink_.getHandle(), L"Project page");
}
