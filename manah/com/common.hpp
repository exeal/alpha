...
#define MANAH_VERIFY_POINTER(p)	\
	if((p) == 0)				\
		...
		public:
			...
			ComPtr<Interface>(REFCLSID clsid, REFIID iid /* = __uuidof(Interface) */, DWORD context = CLSCTX_ALL,
				IUnknown* outer = 0, HRESULT* hr = 0) : pointee_(0) {const HRESULT r = ::CoCreateInstance(clsid, outer, context, iid, initializePPV()); if(hr != 0) *hr = r;}
			/// Copy-constructor.
			ComPtr(const ComPtr<Interface>& rhs) /*throw()*/ : pointee_(rhs.pointee_) {if(pointee_ != 0) pointee_->AddRef();}
			/// Assignment operator.
			ComPtr<Interface>& operator=(const ComPtr<Interface>& rhs) /*throw()*/ {reset(rhs.get()); return *this;}
			/// Destructor.
		...
		public:
			...
			ComQIPtr(const ComPtr<Other>& rhs) /*throw()*/ {if(rhs.get() != 0) rhs->QueryInterface(*iid, initializePPV());}
			/// Copy-constructor.
			ComQIPtr(const ComQIPtr<Interface, iid>& rhs) /*throw()*/ : ComPtr<Interface>(rhs) {}
			/// Assignment operator.
			ComQIPtr<Interface, iid>& operator=(IUnknown* rhs) /*throw()*/ {
				ComPtr<Interface> temp;
				if(rhs != 0)
					rhs->QueryInterface(*iid, temp.initializePPV());
