...
DefaultContentAssistant::CompletionProposalsPopup::CompletionProposalsPopup(
		TextViewer& parent, ContentAssistant::CompletionProposalsUI& ui) BOOST_NOEXCEPT : ui_(ui) {
	add(view_);
...
void DefaultContentAssistant::CompletionProposalsPopup::selectProposal(shared_ptr<const CompletionProposal> selection) {
	if(const Glib::RefPtr<Gtk::TreeSelection> s = view_.get_selection()) {
		...
		BOOST_FOREACH(auto i, model_->children()) {
			if(static_cast<shared_ptr<const CompletionProposal>>((*i)[columns_.proposalObject]) == selection) {
				s->select(i);
				break;
			}
		}
