...
#include "../../window.hpp"
#include "graphics/graphics-windows.hpp"	// win32.PaintContext
#include "menu.hpp"
	...
	static LRESULT CALLBACK windowProcedure(HWND window, UINT message, WPARAM wp, LPARAM lp) {
		...
		WindowBase* const self = reinterpret_cast<WindowBase*>(
			...
			if(handled)
				...
				PaintContext context(Handle<HWND>(window));
				self->paint(context);
				return FALSE;
			}
			return self->fireProcessWindowMessage(message, wp, lp);
...
public:
	...
	void scheduleRedraw(bool eraseBackground) {
		if(!boole(::InvalidateRect(handle().use(), 0, eraseBackground)))
			throw PlatformDependentError<>();
	}
	void scheduleRedraw(const graphics::Rect<>& rect, bool eraseBackground) {
		RECT temp = toNative(rect);
		if(!boole(::InvalidateRect(handle().use(), &temp, eraseBackground)))
			throw PlatformDependentError<>();
	}
	...
	void scrollInformation(int bar, SCROLLINFO& scrollInfo, UINT mask = SIF_ALL) const {
		...
		if(!boole(::GetScrollInfo(handle().use(), bar, &scrollInfo)))
			throw PlatformDependentError<>();
	}
	int scrollPosition(int bar) const {return ::GetScrollPos(handle().use(), bar);}
	Range<int> scrollRange(int bar) const {
		int minPos, maxPos;
		if(!boole(::GetScrollRange(handle().use(), bar, &minPos, &maxPos)))
			throw PlatformDependentError<>();
	...
	void setScrollInformation(int bar, const SCROLLINFO& scrollInfo, bool redraw = true) {
		if(!boole(::SetScrollInfo(handle().use(), bar, &scrollInfo, redraw)))
			throw PlatformDependentError<>();
	}
	int setScrollPosition(int bar, int pos, bool redraw = true) {
		return ::SetScrollPos(handle().use(), bar, pos, redraw);
	}
	void Window::setScrollRange(int bar, const Range<int>& range, bool redraw = true) {
		::SetScrollRange(handle().use(), bar, range.beginning(), range.end(), redraw);
	}
	// viewers.Window implementation
	graphics::Rect<> bounds(bool includeFrame) const {
		RECT temp;
		if(includeFrame) {
			if(!boole(::GetWindowRect(handle().use(), &temp)))
				throw PlatformDependentError<>();
		} else {
			if(!boole(::GetClientRect(handle().use(), &temp)))
				throw PlatformDependentError<>();
		}
		return graphics::fromNative(temp);
	}
	bool hasFocus() const /*throw()*/ {
		return ::GetFocus() == handle().use();
	}
	void hide() {
		if(!boole(::SetWindowPos(handle().use(), 0, 0, 0, 0, 0,
	...
	void redrawScheduledRegion() {
		if(!boole(::UpdateWindow(handle().use())))
			throw PlatformDependentError<>();
	}
	void setBounds(const graphics::Rect<>& bounds) {
		if(!boole(::SetWindowPos(handle().use(), 0,
				bounds.origin().x, bounds.origin().y,
				bounds.size().cx, bounds.size().cy, SWP_NOACTIVATE | SWP_NOZORDER)))
			throw PlatformDependentError<>();
	}
	void show() {
		if(!boole(::ShowWindow(handle().use(), SW_SHOWNOACTIVATE)))
			throw PlatformDependentError<>();
	}
private:
};
