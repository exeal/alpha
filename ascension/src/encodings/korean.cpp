...
#include "generated/windows-949-to-ucs.dat"
					...
					private:
						...
						const EncodingProperties& properties() const BOOST_NOEXCEPT {return properties_;}
					...
					public:
						Uhc() BOOST_NOEXCEPT : EncoderFactoryBase("UHC", standard::UHC, "Korean (UHC)", 2, 1,
					...
					class EucKr : public EncoderFactoryBase {
					public:
						EucKr() BOOST_NOEXCEPT : EncoderFactoryBase("EUC-KR", standard::EUC_KR, "Korean (EUC-KR)", 2, 1,
					...
					class Iso2022Kr : public EncoderFactoryBase {
					public:
						Iso2022Kr() BOOST_NOEXCEPT : EncoderFactoryBase("ISO-2022-KR", standard::ISO_2022_KR, "Korean (ISO-2022-KR)", 7, 1, "csISO2022KR") {}
					private:
						std::unique_ptr<Encoder> create() const BOOST_NOEXCEPT {
					...
					struct Installer {
						Installer() {
							Encoder::registerFactory(std::make_shared<const Uhc>());
							Encoder::registerFactory(std::make_shared<const EucKr>());
							Encoder::registerFactory(std::make_shared<const Iso2022Kr>());
						}
					} installer;
					...
					template<> Encoder::Result InternalEncoder<EucKr>::doFromUnicode(
							Byte* to, Byte* toEnd, Byte*& toNext, const Char* from, const Char* fromEnd, const Char*& fromNext) {
						for(; to < toEnd && from < fromEnd; ++from) {
							if(*from < 0x80)
								*(to++) = mask8Bit(*from);
...
//									if(lead >= 0xa1 && lead <= 0xfe && trail >= 0xa1 && trail <= 0xfe) {
					...
					template<> Encoder::Result InternalEncoder<EucKr>::doToUnicode(
							Char* to, Char* toEnd, Char*& toNext, const Byte* from, const Byte* fromEnd, const Byte*& fromNext) {
						while(to < toEnd && from < fromEnd) {
							if(*from < 0x80)
								*(to++) = *(from++);
...
//							if(!(from[0] >= 0xa1 && from[0] <= 0xfe) || !(from[1] >= 0xa1 && from[1] <= 0xfe)) {
					...
					template<> Encoder::Result InternalEncoder<Iso2022Kr>::doFromUnicode(
							Byte* to, Byte* toEnd, Byte*& toNext, const Char* from, const Char* fromEnd, const Char*& fromNext) {
						if(encodingState_ == 0) {
...
//									if(lead >= 0xa1 && lead <= 0xfe && trail >= 0xa1 && trail <= 0xfe) {
					...
					template<> Encoder::Result InternalEncoder<Iso2022Kr>::doToUnicode(
							Char* to, Char* toEnd, Char*& toNext, const Byte* from, const Byte* fromEnd, const Byte*& fromNext) {
						if(decodingState_ == 0)
							++decodingState_;
						while(to < toEnd && from < fromEnd) {
